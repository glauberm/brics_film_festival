...
class Preservation_Meeting_Enrollment_Form extends Custom_Form {
	protected static $path           = '/pt/inscricao-encontro-preservacao';
	protected static $sitename       = '4º Festival de Cinema BRICS';
	protected static $email_template = 'preservation-meeting-enrollment-email';
	...
	public static function init() {
		add_action( 'rest_api_init', array( 'Preservation_Meeting_Enrollment_Form', 'api_route' ) );
	}
	...
	public static function api_route() {
		register_rest_route(
			'brics/v1', // namespace
			'preservation_meeting_enrollment', // route
			array( // args
				'methods'  => 'POST',
				'callback' => array( 'Preservation_Meeting_Enrollment_Form', 'handle_form' ),
			)
		);
	}
	...
	// 	$query = "SELECT COUNT(*)
	// 		FROM {$wpdb->prefix}preservation_meeting_enrollment
	// 		WHERE course = '$course'";
	...
	private static function insert_row( $name, $age, $profession, $work_field, $formation, $knowledge, $address, $telephone, $email, $description, $interests ) {
		...
		$table_name = "{$wpdb->prefix}preservation_meeting_enrollment";
	...
	private static function send_mail( $name, $age, $profession, $work_field, $formation, $knowledge, $address, $telephone, $email, $description, $interests ) {
		$subject = 'Inscrição no Encontro de Preservação: ' . $name;
		wp_mail(
